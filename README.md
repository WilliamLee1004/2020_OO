...
### 摘要: 近幾年網銀有越來越流行的趨勢，因為它可以讓使用者迅速處理他所需的業務，且不須時間、地點的限制，又可以降低人力需求，假如使用者要開戶，可直接在線上開戶，而不用帶一堆證件去銀行，這些都是純網銀越來越流行的原因。本系統是針對線上網路銀行為主，所以本次軟體所開發出的平台主要會以網頁架構來呈現，讓客戶或銀行職員在線上就能處理相關業務工作；目前的設計方法主要是以物件導向的設計工作執行，讓整體的設計工作能夠簡單且能夠將重複功能再次利用，這樣不僅能夠提升開發速度又能夠兼顧其品質。因此我們制訂計劃來做這一套網銀管理的系統。
...
| 欄位名稱 | 備註說明 |
| -------- |  --- |
| 職員     | 每個職員都有自己對應的客戶，因此此欄位為readonly。幫客戶開戶前，必須先登入。  |
| 姓名     |  不得為null  |
| 密碼     |   不得為null  |
| 確認密碼 |  不得為null且要與密碼欄位一致  |
| 電話    |  不得為null  |
| 地址    |  不得為null   |
| 開戶存款 |  不得為null   |
